...
function buildMekamount(sol, mekaNFT, pfpNFT, pfpScale, success, failure){
  ...
  .then((owners)=>{
    ...
    Promise.all([
      ...
      let pfpFilePath = `./img/${sol}_pfp.png`;
      ...
      .then(() => {
        ...
        .then(()=>{
          ...
          .then(()=>{
            mergeIMG(mekFilePath, pfpMiniAlphaFilePath, finalFilePath, pfpCenterX - (getPFPSize(pfpScale) / 2), pfpBottomY - getPFPSize(pfpScale))
            .then(()=>{
              cleanIMG([pfpFilePath, mekFilePath, pfpAlphaFilePath, pfpMiniAlphaFilePath]);
